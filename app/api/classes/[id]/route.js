...
export async function GET(req, {params}) {
    ...
    await dbConnect();
    try {
        const class_ = await Class.findById(classCode).select("-student_ids").populate("groups", "-student_ids");
        if (!class_) {
            return NextResponse.json({message: "Class not found"});
        }
...
export async function PATCH(req, { params }) {
    const classCode = params.id;
    const { dataParsed } = await req.json();
    await dbConnect();
    try {
        const { className, groups } = dataParsed;
